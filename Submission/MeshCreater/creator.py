...
edges_ti = ti.field(dtype=ti.i32,
                       shape=(max_num_particles, 2))
...
def new_particle(pos_x: ti.f32, pos_y: ti.f32, fixed_: ti.i32):
    ...
    for i in range(new_particle_id):
        ...
        if dist < connection_radius:
            ...
            edges_ti[num_edges[None], 0] = i
            edges_ti[num_edges[None], 1] = new_particle_id
            num_edges[None] += 1
    ...
    while True:
        for e in gui.get_events(ti.GUI.PRESS):
            if e.key in [ti.GUI.ESCAPE, ti.GUI.EXIT]:
                ...
                pos_len = num_particles.to_numpy()
                pos = x.to_numpy()[:pos_len]
